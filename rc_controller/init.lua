--RC Controller!!
--See mesecons_luacontroller/init.lua

local BASENAME = "rc_controller:rc_controller"
local RC_RADIUS = 5
...
local function get_interrupt(pos)
	...
	local function interrupt(time, iid)
		...
		mesecon.queue:add_action(pos, "rc_interrupt", {luac_id, iid}, time, iid, 1)
	end
	return interrupt
end
...
local function get_rc_send(pos)
	if not rc then return end
	local meta = minetest.get_meta(pos)
	sender = minetest.get_player_by_name(meta:get_string("owner") or "")
	return function(to_pos, msg)
	        if not sender then return end
		minetest.after(0, function()
			rc.send(RC_RADIUS, pos, to_pos, msg, sender)
		end)
	end
end
...
local function run(pos, event)
	...
	local success, msg = pcall(f) --RC swap to luacontroller !!
...
mesecon.queue:add_function("rc_interrupt", function (pos, luac_id, iid)
	...
	minetest.register_node(node_name, {
		...
		rc = {
		        receive = function(pos, msg, sender)
			        run(pos, {type = "rc_msg", msg = msg, sender = sender:get_player_name() or ""})
			end
		},
